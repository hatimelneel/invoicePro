...
async function createSinglePagePDFBlob(){
  ...
  const pdfBytes = jpegToSinglePagePDF(jpegBytes,canvas.width,canvas.height);
  return new Blob([pdfBytes],{type:"application/pdf"});
}

async function openSinglePagePDF(){
  if(!previewDoc){
    alert(tr("Nothing to export.","لا يوجد مستند للتصدير."));
    return;
  }

  const button = document.querySelector(".pdf-main-btn");
  try{
    if(button){
      button.disabled = true;
      button.dataset.originalText = button.textContent;
      button.textContent = tr("Creating PDF…","جاري إنشاء PDF…");
    }

    const blob = await createSinglePagePDFBlob();
    const url = URL.createObjectURL(blob);

    // Navigate directly to the actual generated PDF.
    window.location.assign(url);

    // Keep it alive long enough for iPhone PDF viewer/share sheet.
    setTimeout(()=>URL.revokeObjectURL(url), 15 * 60 * 1000);
  }catch(err){
    console.error(err);
    if(button){
      button.disabled = false;
      button.textContent = button.dataset.originalText || tr("Open PDF","فتح PDF");
    }
    alert(tr("Could not create PDF.","تعذر إنشاء ملف PDF."));
  }
}

function printPreview(){ return openSinglePagePDF(); }
function openPrintView(){ return openSinglePagePDF(); }
async function openNativePDF(){ return openSinglePagePDF(); }
async function sharePDFFile(){ return openSinglePagePDF(); }
