...
function show(id){
  ...
  if(id==="documents"){
    ...
    if(typeof installDocumentTabHandlers==="function") installDocumentTabHandlers();
...
function installDocumentTabHandlers(){
  ...
  if(quoteBtn && !quoteBtn.dataset.bound){
    ...
    quoteBtn.addEventListener("click",function(ev){
      ev.preventDefault();
      currentDocumentTab="quotes";
      setDocumentTab("quotes");
    });
  }
}

function showRestoreSuccessOnce(){
  const details=localStorage.getItem("invoicepro_restore_success");
  if(!details) return;
  localStorage.removeItem("invoicepro_restore_success");

  // Allow the freshly reloaded interface to finish rendering first.
  setTimeout(()=>{
    console.log("Restore completed:",details);
  },100);
}

setLang(lang);renderHome();populate();updateTotals();installDocumentTabHandlers();showRestoreSuccessOnce();initAutoDailyBackup();
...
function renderDocuments(){
  ...
  const safeBadge=(doc,type)=>{
    try{
      const status=type==="quote"?quoteDisplayStatus(doc):invoiceDisplayStatus(doc);
      return statusBadge(status);
    }catch(e){
      console.error("Status badge error",e,doc);
      return "";
    }
  };
  ...
  source.forEach((raw,i)=>{
    try{
      ...
      const number=esc(x.number||((currentDocumentTab==="quotes"?"QT-":"INV-")+String(i+1).padStart(5,"0")));
      const customer=esc(x.customer||"");
      const total=money(+x.total||0);
          ...
          <div class="document-row">
            ...
            <div class="document-side">
              ...
              <div class="document-row-actions">
                <button type="button" onclick="openSavedQuote(${i})">${tr("Preview","معاينة")}</button>
                <button type="button" onclick="editSavedQuote(${i})">${tr("Edit","تعديل")}</button>
                ${x.convertedInvoiceNumber
                  ?`<button type="button" class="converted" onclick="openConvertedInvoice('${esc(x.convertedInvoiceNumber)}')">${tr("Open Invoice","فتح الفاتورة")}</button>`
                  :`<button type="button" class="convert" onclick="convertQuoteToInvoice(${i})">${tr("Convert","تحويل لفاتورة")}</button>`}
              </div>
            </div>
          </div>`);
      }else{
        rows.push(`
          <div class="document-row">
            ...
            <div class="document-side">
              <strong>${total}</strong>
              <div class="document-row-actions">
                <button type="button" onclick="openSavedInvoice(${i})">${tr("Preview","معاينة")}</button>
                <button type="button" onclick="editSavedInvoice(${i})">${tr("Edit","تعديل")}</button>
              </div>
            </div>
          </div>`);
      }
    }catch(e){
      console.error("Document row error:",e,raw);
    }
  });
...
function importBackupFile(event){
  ...
  if(file.size>0 && file.size<80){
    alert(tr(
      "This backup file is incomplete ("+file.size+" bytes). Please choose the larger Invoice Pro JSON backup file.",
      "ملف النسخة الاحتياطية غير مكتمل ("+file.size+" بايت). اختر ملف Invoice Pro JSON الأكبر حجماً."
    ));
    event.target.value="";
    return;
  }
  ...
  reader.onerror=()=>{
    alert(tr(
      "The selected file could not be read. If it is stored in iCloud or Google Drive, download it first and try again.",
      "تعذر قراءة الملف المحدد. إذا كان في iCloud أو Google Drive، قم بتنزيله أولاً ثم حاول مرة أخرى."
    ));
    event.target.value="";
  };
  ...
  reader.onload=()=>{
    ...
    // Phase 1: parse and validate only.
    try{
      parsed=parseBackupJSON(reader.result);
      const rawIncoming=extractBackupData(parsed);
      incoming=migrateBackupData(rawIncoming);
      summary=backupContentsSummary(incoming);
    }catch(e){
      console.error("Backup parse error:",e);

      let message=tr(
        "This file could not be read as an Invoice Pro backup.",
        "تعذر قراءة هذا الملف كنسخة احتياطية من Invoice Pro."
      );

      if(e && (e.message==="EMPTY_BACKUP" || e instanceof SyntaxError)){
        message=tr(
          "The selected JSON file is empty or damaged. Please choose another Invoice Pro backup file.",
          "ملف JSON المحدد فارغ أو تالف. اختر نسخة احتياطية أخرى من Invoice Pro."
        );
      }
    ...
    if(!confirm(tr(
      "Backup found: "+details+". Restore it now? Current local data will be replaced.",
      "تم العثور على نسخة تحتوي على: "+details+". هل تريد استعادتها الآن؟ سيتم استبدال البيانات الحالية."
    ))){
      event.target.value="";
      return;
    }

    // Phase 2: save restored database only.
    try{
      db=incoming;
      normalizeDB();
      saveDB();
      ...
      localStorage.setItem("invoicepro_restore_success",details);
    }catch(e){
      console.error("Backup save error:",e);
      alert(tr(
        "The backup was read, but the restored data could not be saved on this device.",
        "تمت قراءة النسخة، لكن تعذر حفظ البيانات المستعادة على هذا الجهاز."
      ));
      event.target.value="";
      return;
    }
    ...
    // Important: do NOT rebuild all screens inside the restore callback.
    // iOS Safari can retain stale DOM references after replacing the database.
    alert(tr(
      "Backup restored successfully. The app will reload now.",
      "تمت استعادة النسخة الاحتياطية بنجاح. سيتم إعادة تشغيل التطبيق الآن."
    ));
